def rectum_seg(rootDir):  
  ...
  dicomDataDir = os.path.join(rootDir, subDirs[scanNumber],"dicom") 
  ...
  # Export seg.nrrd
  segmentDir = os.path.join("/",*rootDir.split("/")[:-1], "rectum_segmentation", subDirs[scanNumber])
  if not os.path.exists(segmentDir):
      os.makedirs(segmentDir)
  
  # Save segmentation and its dicom files as .nrrd
  segmentPath = os.path.join(segmentDir,subDirs[scanNumber][:30] + ".seg.nrrd")
  volumnPath  = os.path.join(segmentDir,subDirs[scanNumber] + ".nrrd")
